...
int	validate_map_dimensions(t_parse *data, t_map *map, int start_line)
{
	...
	if (map->height > MAX_ROWS)
	{
		printf("Error: Map height (%d) exceeds MAX_ROWS (%d)\n",
			map->height, MAX_ROWS);
		return (0);
	}
	map->width = find_max_width(data->file_content, start_line);
	if (map->width == 0 || map->width > MAX_COLUMS)
	{
		printf("Error: Map width (%d) invalid or exceeds MAX_COLUMS (%d)\n",
			map->width, MAX_COLUMS);
		return (0);
	}
	return (1);
}
...
int	parse_map_grid(t_parse *data, t_map *map, int start_line)
{
	...
	while (data->file_content[i] && j < map->height)
	{
		...
		if (!find_player_position(map->map_grids[j], &map->player, j))
			return (0);
		i++;
		j++;
	}
